"""Home Assistant MQTT Discovery for vidaa2mqtt."""
...
def get_device_info(config: dict, device_id: str) -> dict:
    ...
    return {
        "identifiers": [f"vidaa_{device_id}"],
        "name": tv_config.get("name", "Vidaa TV"),
...
def get_availability(device_id: str) -> list[dict]:
    ...
    return [
        {
            "topic": f"vidaa2mqtt/{device_id}/state/available",
...
def generate_media_player_discovery(config: dict, device_id: str, discovery_prefix: str) -> tuple[str, dict]:
    ...
    """
    topic = f"{discovery_prefix}/media_player/vidaa_{device_id}/config"

    sources = ["TV", "HDMI1", "HDMI2", "HDMI3", "HDMI4", "AV", "Component"]

    payload = {
        "name": None,  # Use device name
        "unique_id": f"vidaa_{device_id}_media_player",
        "object_id": f"vidaa_{device_id}",
        "device": get_device_info(config, device_id),
        "availability": get_availability(device_id),
        # State - use "on"/"off" for proper logbook events
        "state_topic": f"vidaa2mqtt/{device_id}/state/power",
        "state_value_template": "{{ 'on' if value == 'ON' else 'off' }}",
        # Commands
        "command_topic": f"vidaa2mqtt/{device_id}/set/power",
        "payload_on": "ON",
        "payload_off": "OFF",
        # Volume
        "volume_level_topic": f"vidaa2mqtt/{device_id}/state/volume",
        "volume_level_template": "{{ value | float / 100 }}",
        "set_volume_topic": f"vidaa2mqtt/{device_id}/set/volume",
        # Mute
        "mute_state_topic": f"vidaa2mqtt/{device_id}/state/mute",
        "mute_command_topic": f"vidaa2mqtt/{device_id}/set/mute",
        "payload_mute_on": "ON",
        "payload_mute_off": "OFF",
        # Source / Current App
        "source_list": sources,
        "source_topic": f"vidaa2mqtt/{device_id}/state/source",
        "source_command_topic": f"vidaa2mqtt/{device_id}/set/source",
        # Media info - shows current app
        "media_title_topic": f"vidaa2mqtt/{device_id}/state/app",
...
def generate_power_switch_discovery(config: dict, device_id: str, discovery_prefix: str) -> tuple[str, dict]:
    ...
    topic = f"{discovery_prefix}/switch/vidaa_{device_id}_power/config"

    payload = {
        "name": "Power",
        "unique_id": f"vidaa_{device_id}_power",
        "object_id": f"vidaa_{device_id}_power",
        "device": get_device_info(config, device_id),
        "availability": get_availability(device_id),
        "state_topic": f"vidaa2mqtt/{device_id}/state/power",
        "command_topic": f"vidaa2mqtt/{device_id}/set/power",
...
def generate_app_sensor_discovery(config: dict, device_id: str, discovery_prefix: str) -> tuple[str, dict]:
    ...
    topic = f"{discovery_prefix}/sensor/vidaa_{device_id}_app/config"

    payload = {
        "name": "Current App",
        "unique_id": f"vidaa_{device_id}_current_app",
        "object_id": f"vidaa_{device_id}_current_app",
        "device": get_device_info(config, device_id),
        "availability": get_availability(device_id),
        "state_topic": f"vidaa2mqtt/{device_id}/state/app",
...
def generate_volume_discovery(config: dict, device_id: str, discovery_prefix: str) -> tuple[str, dict]:
    ...
    topic = f"{discovery_prefix}/number/vidaa_{device_id}_volume/config"

    payload = {
        "name": "Volume",
        "unique_id": f"vidaa_{device_id}_volume",
        "object_id": f"vidaa_{device_id}_volume",
        "device": get_device_info(config, device_id),
        "availability": get_availability(device_id),
        "state_topic": f"vidaa2mqtt/{device_id}/state/volume",
        "command_topic": f"vidaa2mqtt/{device_id}/set/volume",
        "min": 0,
        "max": 100,
        "step": 1,
        "unit_of_measurement": "%",
...
def generate_mute_switch_discovery(config: dict, device_id: str, discovery_prefix: str) -> tuple[str, dict]:
    ...
    topic = f"{discovery_prefix}/switch/vidaa_{device_id}_mute/config"

    payload = {
        "name": "Mute",
        "unique_id": f"vidaa_{device_id}_mute",
        "object_id": f"vidaa_{device_id}_mute",
        "device": get_device_info(config, device_id),
        "availability": get_availability(device_id),
        "state_topic": f"vidaa2mqtt/{device_id}/state/mute",
        "command_topic": f"vidaa2mqtt/{device_id}/set/mute",
        "payload_on": "ON",
        "payload_off": "OFF",
        "state_on": "ON",
        "state_off": "OFF",
...
def generate_button_discovery(
    ...
    topic = f"{discovery_prefix}/button/vidaa_{device_id}_{button_id}/config"

    payload = {
        "name": name,
        "unique_id": f"vidaa_{device_id}_{button_id}",
        "object_id": f"vidaa_{device_id}_{button_id}",
        "device": get_device_info(config, device_id),
        "availability": get_availability(device_id),
        "command_topic": f"vidaa2mqtt/{device_id}/set/key",
...
def generate_select_discovery(
    ...
    topic = f"{discovery_prefix}/select/vidaa_{device_id}_app/config"

    payload = {
        "name": "Launch App",
        "unique_id": f"vidaa_{device_id}_app",
        "object_id": f"vidaa_{device_id}_app",
        "device": get_device_info(config, device_id),
        "availability": get_availability(device_id),
        "command_topic": f"vidaa2mqtt/{device_id}/set/app",
        "options": apps,
        "icon": "mdi:apps",
    }
...
def remove_all_discoveries(config: dict, device_id: str) -> list[str]:
    ...
    topics = [
        f"{discovery_prefix}/media_player/vidaa_{device_id}/config",
        f"{discovery_prefix}/switch/vidaa_{device_id}_power/config",
        f"{discovery_prefix}/number/vidaa_{device_id}_volume/config",
        f"{discovery_prefix}/switch/vidaa_{device_id}_mute/config",
        f"{discovery_prefix}/sensor/vidaa_{device_id}_app/config",
        f"{discovery_prefix}/select/vidaa_{device_id}_app/config",
        # Legacy binary_sensor (in case upgrading from old version)
        f"{discovery_prefix}/binary_sensor/vidaa_{device_id}_power/config",
    ]

    nav_buttons = ["up", "down", "left", "right", "ok", "back", "home", "menu"]
    for button_id in nav_buttons:
        topics.append(f"{discovery_prefix}/button/vidaa_{device_id}_{button_id}/config")

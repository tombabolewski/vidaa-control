"""Main bridge class for vidaa2mqtt."""
...
class VidaaMQTTBridge:
    ...
    def _setup_broker_client(self):
        ...
        base_id = mqtt_config.get("client_id", "vidaa2mqtt")
        ...
        self._broker_client.will_set(
            f"vidaa2mqtt/{self.device_id}/state/available",
            payload="offline",
            qos=1,
            retain=True,
        )
    ...
    def _on_broker_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            base = f"vidaa2mqtt/{self.device_id}/set"
            topics = [
                (f"{base}/power", 0),
                (f"{base}/volume", 0),
                (f"{base}/mute", 0),
    ...
    def _publish_state(self, state_type: str, value: str):
        ...
        if self._broker_client and self._broker_client.is_connected():
            topic = f"vidaa2mqtt/{self.device_id}/state/{state_type}"
    ...
    def _publish_availability(self, available: bool):
        ...
        if self._broker_client and self._broker_client.is_connected():
            topic = f"vidaa2mqtt/{self.device_id}/state/available"
    ...
    def start(self):
        """Start the bridge."""
        logger.info("Starting vidaa2mqtt bridge...")
        ...
        logger.info("vidaa2mqtt bridge started")

    def stop(self):
        """Stop the bridge."""
        logger.info("Stopping vidaa2mqtt bridge...")
        self.running = False
        ...
        logger.info("vidaa2mqtt bridge stopped")

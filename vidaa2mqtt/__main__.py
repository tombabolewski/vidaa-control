...
from .bridge import VidaaMQTTBridge
...
def main():
    ...
    parser.add_argument(
        "-v", "--version",
        action="version",
        version=f"vidaa2mqtt {__version__}",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    # Start bridge
    logger.info(f"vidaa2mqtt v{__version__} starting...")

    bridge = VidaaMQTTBridge(config)

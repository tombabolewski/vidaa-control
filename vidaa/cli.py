#!/usr/bin/env python3
"""Command-line interface for Vidaa TV control."""
...
def cmd_discover(args):
    """Discover Vidaa TVs on the network."""
    timeout = getattr(args, 'timeout', 5.0)
    method = getattr(args, 'method', 'all')
    verbose = getattr(args, 'verbose', False)

    print(f"Scanning for Vidaa TVs (timeout: {timeout}s)...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="tv",
        description="Control your Vidaa TV from the command line",
    )
